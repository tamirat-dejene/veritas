...
func (uc *monitoringUseCase) ListSessionsForExam(ctx context.Context, examID uuid.UUID, enterpriseID uuid.UUID, status *domain.SessionStatus, candidateID *uuid.UUID) ([]*domain.ExamSession, error) {
	...
	return uc.sessionRepo.ListSessionsByExam(ctx, examID, status)
}
...
func (uc *monitoringUseCase) GetSubmissions(ctx context.Context, examID uuid.UUID, enterpriseID uuid.UUID) ([]*domain.ExamSubmission, error) {
	return uc.sessionRepo.GetSubmissionsByExam(ctx, examID)
}
