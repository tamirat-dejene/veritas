...
type EnterpriseSubscription struct {
	...
	// Chapa-specific fields (populated when payment_provider = "chapa")
	ChapaTxRef      *string `db:"chapa_tx_ref" json:"chapa_tx_ref,omitempty"`
...
// CheckoutRequest is a provider-agnostic request to create a payment/checkout session.
type CheckoutRequest struct {
	EnterpriseID uuid.UUID
	Plan         *SubscriptionPlan
	// CustomerRef holds the Stripe customer ID (empty for Chapa).
	CustomerRef *string
	// TxRef is a unique reference generated by Veritas; required by Chapa, ignored by Stripe.
	TxRef string
	// ReturnURL is where the user is redirected after payment completion.
	ReturnURL string
	// CallbackURL is where the provider sends the async payment notification.
	CallbackURL string
}

// PaymentEvent is a provider-agnostic representation of a webhook/callback event.
type PaymentEvent struct {
	// EventID is used for idempotency deduplication.
	EventID string
	// EventType is one of: "checkout.completed", "payment.success", "payment.failed".
	EventType string
	// TxRef is the Veritas-generated reference (used as EventID for Chapa).
	TxRef string
	EnterpriseID    uuid.UUID
	PlanID          uuid.UUID
	Amount          float64
	Currency        Currency
	// CustomerRef holds the provider customer ID (Stripe) or empty string (Chapa).
	CustomerRef string
	// SubscriptionRef holds the provider subscription ID (Stripe) or empty string (Chapa).
